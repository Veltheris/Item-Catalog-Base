...
from alchemy_setup import app, db, Category, Item, User
...
# Old - Setup for the SQLAlchemy db.session
...
if DEBUG:
 ...
 def catalogTestData():
    userOne = User(name='Test Data', email='Test@Data', id='0', picture='')
    db.session.add(userOne)
    db.session.commit()
    catOne = Category(name='Wargame', user_id='0') 
    db.session.add(catOne)
    catTwo = Category(name='Roleplaying', user_id='0')
    db.session.add(catTwo)
    catThree = Category(name='Strategy', user_id='0')
    db.session.add(catThree)
    catFour = Category(name='Other', user_id='0', public=False)
    db.session.add(catFour)
    db.session.commit()
...
def catalogLogin():
   ...
   # return "The Current db.session state is %s" % (login_session['state'],)
...
# Disconnect from Google and Wipe the db.session
...
def catalogListHome():
   # Latest 5 Items
    items = db.session.query(Item).join('category').order_by(db.desc(Item.changed)).limit(5)
    category = db.session.query(Category).all()
    itemcount = db.session.query(Item)
...
def catalogDelCategory(category):
    ...
    if request.method == 'GET':
       # Use the Login db.session to store a token to reduce CSRF.
...
def catalogListCategory(category):
    ...
    sidebar = db.session.query(Category).all()
    itemcount = db.session.query(Item)
...
def catalogAddItem():
    if request.method == 'GET':
        ...
        category = db.session.query(Category).get(newCategory)
...
def catalogEditItem(item):
    item = db.session.query(Item).get(item)
    ...
    if request.method == 'GET':
        ...
        return redirect(url_for('.catalogListCategory', category=db.session.query(Category).get(newCategory).name))
...
def catalogDelItem(item):
    item = db.session.query(Item).get(item)
    category = db.session.query(Category).get(item.category_id)
...
def catalogRSSAll():
  items = db.session.query(Item).order_by(db.desc(Item.changed)).all()
...
def catalogRSSCategory(category):
  category = db.session.query(Category).filter_by(name = category).first()
  items = db.session.query(Item).filter_by(category_id=category.id).order_by(db.desc(Item.changed)).all()
  url = FullURL
  latest = items[0].changed.strftime('%d %B %Y %X')
